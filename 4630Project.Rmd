...
metabric <- read.csv("metabric.csv",
...
# EDA of Scatterplots with Outcome
...
# EDA for Outlier Check of Clinical Variables
...
p_calib <- ggplot(calib_all, aes(x = mean_pred, y = obs_rate, color = model)) +
  geom_line() +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(
...
# -------------------------------
p_overall <- ggplot(prob_long, aes(x = prob, fill = model)) +
  geom_density(alpha = 0.4) +
  labs(
...
ggsave(
  filename = "prob_distribution_models.png",
  plot = p_overall,
  width = 8,
  height = 6,
  dpi = 300
...
# -------------------------------
# 2. Distributions split by true class
# -------------------------------
prob_long$y_true_fac <- factor(
  prob_long$y_true,
  levels = c(0, 1),
  labels = c("Deceased (0)", "Alive (1)")
)

p_byclass <- ggplot(prob_long, aes(x = prob, fill = model)) +
  geom_density(alpha = 0.4) +
...
ggsave(
  filename = "prob_distribution_by_class.png",
  plot = p_byclass,
  width = 8,
  height = 8,
  dpi = 300
...
# --- Build the plot ---
p_imp <- ggplot(imp_plot_df, aes(
  x = reorder(feature, importance_scaled),
  y = importance_scaled,
  fill = model
)) +
...
# --- Save plot as PNG ---
ggsave(
  filename = "variable_importance.png",
  plot = p_imp,
  width = 10,   # horizontal for readability
  height = 8,
  dpi = 300
)
...
# Choose ONE consistent border thickness
border_thickness <- "12x12"    # ← you can change this value if needed
...
image_read("prob_distribution_models.png") %>%
  image_border("black", border_thickness) %>%
  image_write("prob_distribution_models_border.png")

# By-class distribution
image_read("prob_distribution_by_class.png") %>%
  image_border("black", border_thickness) %>%
  image_write("prob_distribution_by_class_border.png")
```

```{r}
# Variable importance (same exact border thickness)
image_read("variable_importance.png") %>%
  image_border("black", border_thickness) %>%
  image_write("variable_importance_border.png")
...
tab_html <- kable(
  model_summary_pretty,
  format = "html",
  caption = "Model Comparison: Performance Metrics"
) %>%
  kable_styling(
...
html_file <- "model_summary_table.html"
save_kable(tab_html, file = html_file)

# Convert to PNG
webshot2::webshot(
  url      = html_file,
  file     = "model_summary_table.png",
  vwidth   = 1200,
  vheight  = 800,
  zoom     = 2
)
